...
public protocol Content: Codable, Equatable { }
...
extension KeyedDecodingContainer {
    ...
    public func decodeArray<T: Decodable>(_ key: K) throws -> [T] {
        ...
        do {
            ...
            if VaporEcobee.safeArrayDecoding {
                return array
            } else {
                throw error
            }
    ...
    fileprivate func decodeOptional<T>(_ closure: () throws -> T? ) throws -> T? {
        if VaporEcobee.safeOptionalDecoding {
            do {
                return try closure()
            } catch {
                return nil
...
extension Date {
    func encode() -> Any {
        return VaporEcobee.dateEncodingFormatter.string(from: self)
    }
}
